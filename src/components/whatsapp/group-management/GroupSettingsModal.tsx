...
export function GroupSettingsModal({ instanceId, groupId, currentSettings, onSuccess, children }: GroupSettingsModalProps) {
  const [open, setOpen] = useState(false);
  const [settings, setSettings] = useState(currentSettings || defaultSettings);
  const [loading, setLoading] = useState(false);

  useEffect(() => {
    if (open) setSettings(currentSettings || defaultSettings);
  ...
  const handleSubmit = async () => {
    ...
    try {
      const { error } = await supabase.functions.invoke("zapi-proxy", {
        body: {
          instanceId,
          endpoint: "/update-group-settings",
          method: "POST",
          body: { phone: groupId, ...settings },
        },
      });
